document.addEventListener('DOMContentLoaded', () => {
    ...
    if (startBtn) {
        startBtn.addEventListener('click', () => {
            if (timerId !== null) return; // Prevent multiple intervals from stacking
            
            // Calculate the exact target millisecond timestamp based on current remaining time
            targetTimestamp = Date.now() + (currentTime * 1000);

            timerId = setInterval(() => {
                const now = Date.now();
                // Calculate remaining seconds precisely from the target timestamp to prevent drift
            ...
            }, 100); // Runs frequently for ultra-smooth display updates without drift
        });
    }
    ...
    if (pauseBtn) {
        pauseBtn.addEventListener('click', () => {
            if (timerId !== null) {
                clearInterval(timerId);
                timerId = null;
                // Freeze the exact remaining time so resuming picks up instantly
                if (targetTimestamp) {
