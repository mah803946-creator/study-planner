document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Ambient Music Player (Local File Safe) ---
    const musicToggleBtn = document.getElementById('music-toggle-btn');
    const nowPlayingEl = document.getElementById('now-playing');
    const musicTrackSelect = document.getElementById('music-track-select');
    
    let currentAudio = new Audio('music.mp3');
    currentAudio.loop = true;

    if (musicToggleBtn && musicTrackSelect) {
        musicToggleBtn.addEventListener('click', () => {
            if (!currentAudio.src.includes(musicTrackSelect.value)) {
                currentAudio.pause();
                currentAudio = new Audio(musicTrackSelect.value);
                currentAudio.loop = true;
            }
            ...
            if (currentAudio.paused) {
                currentAudio.play().then(() => {
                    ...
                    alert("Playback blocked. Make sure 'music.mp3' is uploaded in your repository root folder!");
                });
            } else {
                currentAudio.pause();
